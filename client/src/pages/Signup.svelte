<script lang="ts">
    ...
    const handleSignUp = async () => {
        const response = await axiosPost<AuthRes, ISignUp>('/api/auth/signup', {
            ...signUp,
            role: active.role,
        });
        ...
        try {
            ...
            $notifStore.open(
                `Error saving auth in local storage: ${err.message}`,
                'error'
            );
        }
...
<div class="container">
    <div
        style="background-color: rgb(207, 205, 204, 0.4); padding: 3rem; border-radius: 10px"
    >
        ...
        >
            ...
            <HelperText slot="helper"
                >Password length must be at least 8</HelperText
            >
        </Textfield>
