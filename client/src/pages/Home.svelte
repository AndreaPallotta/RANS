<script lang="ts">
    ...
    const getItems = async () => {
        ...
        if (response.error || !response.data) {
            $notifStore.open(
                response.error ?? 'Error retrieving items',
                'error'
            );
            return;
        }
        ...
        itemsStore.set(
            response.data.content.sort((a: Item, b: Item) => {
    ...
    const handleToggle = async (e: CustomEvent<boolean>) => {
        if (e.detail) {
            itemsToDisplay = $itemsStore.filter(
                ({ user_id }: Item) => user_id === $authStore._key
            );
        } else {
            itemsToDisplay = $itemsStore;
        }
    ...
    const saveEditedItem = async (e: CustomEvent<Item>) => {
        const body = objectDifference<UpdateItemReq, Item>(
            selectedItem,
            e.detail,
            {
                id: selectedItem._key,
        ...
        const response = await axiosPut<Item, UpdateItemReq>(
            '/api/edit_item',
            body
        );
    ...
    const addNewItem = async (e: CustomEvent<Partial<Item>>) => {
        const response = await axiosPost<Item, AddItemReq>('/api/add_item', {
            ...e.detail,
        ...
        if (response.error || !response.data) {
            $notifStore.open(
                response.error ?? 'Error creating new item',
                'error'
            );
            return;
        }
