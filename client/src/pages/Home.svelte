<script lang="ts">
  ...
  import type { AddOrderReq, DeleteItemReq, DeleteItemRes, UpdateItemReq } from '../types/ifaces';
  ...
  const orderItem = async (e: CustomEvent<number>) => {
    ...
    const response = await axiosPost<IOrder, AddOrderReq>('/api/add_order', body);
    ...
    ordersStore.update((prev: IOrder[]) => ([
      ...prev,
    ...
    await getItems();
    $notifStore.open(`Successfully ordered ${quantity} for $${price}`, 'success');
    closeOrderModal();
  };
...
<div class="page-container">
    ...
    {#each $itemsStore as item}
        <Cell>
            <ItemCard {item} on:delete={openConfirmModal} on:edit={openModal} on:order={openOrderModal} />
        </Cell>
    {/each}
    </LayoutGrid>
    ...
    {#if selectedItem && orderOpen}
      <OrderModal open={orderOpen} item={selectedItem} on:cancel={closeOrderModal} on:confirm={orderItem} />
    {/if}
</div>