  ...
  <Content id="order-modal-content">
    ...
    >
      <HelperText slot="helper"
        >Note: Orders must be greater than 0 and less than the total amount</HelperText
      >
    </Textfield>
    <pre>Remaining quantity: {isNaN(item.quantity - quantity)
        ? item.quantity
        : item.quantity - quantity}</pre>
    <pre>Total cost: ${(isNaN(quantity * item.price)
        ? 0.0
        : quantity * item.price
      ).toFixed(2)}</pre>
  </Content>
